...
exports.relatarErro = (mensagem, prefixo, comando, data, hora) => {
return `⚠️ *Erro Detectado no Bot* ⚠️\n\n🔍 *Detalhes do Erro:*\n- Mensagem: ${mensagem}\n- Comando: ${prefixo + comando}\n- Data e Hora: ${data} | ${hora}\n\n🌟 *Ação Recomendada:*\nVerifique os logs e corrija o problema o mais rápido possível.`
}
...
exports.playResult = (data) => {
  return `• Título: *${data.title || "Não encontrado."}*\n• Descrição: *${data.description || "Não encontrado."}*\n• Duração: *${data.timestamp + ' minutos' || "Não encontrado."}*\n• Vídeo URL: *${data.url}*\n—\n> *Informações ~ Vídeo*\n• Visualizações: *${data.views || "Não encontrado."}*\n• Nome do Canal: *${data.author.name || "Não encontrado."}*\n• Canal URL: *${data.author.url || "Não encontrado."}*\n• Publicação: *${data.ago || "Não encontrado."}*`
}

exports.spotifyResult = (data, query, converter) => {
...
exports.mensagemPriv = () => {
return 'Só um minutinho, estou preparando tudo com carinho para você!'
}

exports.mensagemGroup = () => {
return 'Por segurança, mandei o que você pediu no seu privado. Dá uma olhadinha lá!'
}
...
exports.error = () => {
return `Desculpe, ocorreu um erro inesperado. Por favor, tente novamente mais tarde. Este incidente foi registrado e relatado ao meu dono para análise.`
}
